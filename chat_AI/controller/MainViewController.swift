...
class MainViewController: UIViewController, UITextFieldDelegate {
    ...
    let viewModel = GPTChatViewModel()
    var balanceCardView = BalanceCardView()
    var sideMenuTableViewController = SideMenuTableViewController()
    
    var isAnimationItemVisible = false // 메시지 응답 로딩애니메이션 토글
    var isRewardedAdLoaded = false
    var containerViewBottomConstraint: NSLayoutConstraint!
    
    // 전송버튼과 로딩뷰의 전환을 위해 전역변수로 설정
    var sendButtonItem: UIBarButtonItem!
    var loadingAnimationItem: UIBarButtonItem!
    let sideMenuWidth: CGFloat = UIScreen.main.bounds.width * 0.8 > 340 ? 340 : UIScreen.main.bounds.width * 0.8
    var sideMenuView: UIView!
    var overlayView: UIView!
    var shouldProcessNotifications = true
    let AdMobRewardADId = Environment.AdMobRewardADId
    
    // Observable의 메모리 누수 방지를 위한 자동 구독해지 기능이라고 생각하면 편할듯
    private let disposeBag = DisposeBag()
    
    let floatingAnimation: CABasicAnimation = {
    ...
    private let bannerContainerView: UIView = {
        let view = UIView()
        view.backgroundColor = .clear // 배경색은 원하는 색상으로 설정
    ...
    private let messageContainerView: UIStackView = {
    ...
    @objc func handleKeyboardNotification(_ notification: Notification) {
        ...
        if let userInfo = notification.userInfo {
            ...
            scrollView.contentOffset.y += isKeyboardShowing ? keyboardFrame.height - view.safeAreaInsets.bottom : view.safeAreaInsets.bottom - keyboardFrame.height
...
//                print(#function,body["role"]!," : \(body["content"]!)")
    ...
    private func removeBubbleLabel(toggle: Bool){
        if let lastView = self.messageContainerView.arrangedSubviews.last, !toggle {
            self.messageContainerView.removeArrangedSubview(lastView)
    ...
    private func addBubbleLabel(_ text: String, isUser: Bool, isInit: Bool = false){
        ...
        messageContainerView.addArrangedSubview(label) // 스택뷰 맨 끝에 추가
...
//        주의할 점은 메인 스레드에서 wait()를 사용할 때 발생할 수 있는 문제입니다. 메인 스레드에서 대기하게 되면 사용자 인터페이스가 멈추고, 애플리케이션이 정상적으로 동작하지 않을 수 있습니다. 따라서 메인 스레드에서는 주의하여 사용해야 합니다. 이와 같은 상황에서는 백그라운드 스레드에서 동기화 작업을 처리하는 것이 좋습니다.
    ...
    private func setupUI() {
        ...
        scrollView.addSubview(messageContainerView)
        ...
        NSLayoutConstraint.activate([
            // containerView
            // 스크린샷용 주석 ###
            containerView.topAnchor.constraint(equalTo: bannerContainerView.bottomAnchor),
//            containerView.topAnchor.constraint(equalTo: topNavigationBar.bottomAnchor),
            ...
            messageContainerView.topAnchor.constraint(equalTo: scrollView.topAnchor, constant: 20),
            messageContainerView.leadingAnchor.constraint(equalTo: scrollView.leadingAnchor),
            messageContainerView.trailingAnchor.constraint(equalTo: scrollView.trailingAnchor),
            messageContainerView.bottomAnchor.constraint(equalTo: scrollView.bottomAnchor, constant: -20),
            messageContainerView.widthAnchor.constraint(equalTo: scrollView.widthAnchor),
            
            // bannerContainerView
            // 스크린샷용 주석 ###
            bannerContainerView.topAnchor.constraint(equalTo: topNavigationBar.bottomAnchor),
            bannerContainerView.heightAnchor.constraint(equalToConstant: 50), // 높이는 원하는 대로 설정
            ...
            // bottomToolbar
            bottomToolbar.topAnchor.constraint(equalTo: scrollView.bottomAnchor),
            bottomToolbar.leadingAnchor.constraint(equalTo: containerView.leadingAnchor),
            bottomToolbar.trailingAnchor.constraint(equalTo: containerView.trailingAnchor),
            bottomToolbar.bottomAnchor.constraint(equalTo: containerView.bottomAnchor),
